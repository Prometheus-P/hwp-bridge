...
> This document may still mention `hwp-web (planned)` as a *planned* component.
...
```bash
# hwp-web (planned)만 빌드
docker build --target hwp-web (planned) -t hwp-bridge/web:latest .
...
# 3. 이미지 빌드 및 푸시
docker build --target hwp-web (planned) -t asia-northeast3-docker.pkg.dev/PROJECT_ID/hwp-bridge/hwp-web (planned):latest .
docker push asia-northeast3-docker.pkg.dev/PROJECT_ID/hwp-bridge/hwp-web (planned):latest

# 4. Cloud Run 배포
gcloud run deploy hwp-web (planned) \
  --image=asia-northeast3-docker.pkg.dev/PROJECT_ID/hwp-bridge/hwp-web (planned):latest \
...
```bash
gcloud run services update hwp-web (planned) \
  --set-env-vars="RUST_LOG=info" \
  --set-env-vars="GOOGLE_CLIENT_ID (planned)=xxx" \
...
spec:
  tls:
    - hosts:
        - api.hwpbridge.io
      secretName: hwp-web (planned)-tls
  rules:
...
# 로그 확인
kubectl logs -f deployment/hwp-web (planned) -n hwp-bridge
```
...
```ini
# /etc/systemd/system/hwp-web (planned).service
[Unit]
...
ExecStart=/opt/hwp-bridge/hwp-web (planned)
Restart=always
RestartSec=5
...
sudo systemctl enable hwp-web (planned)
sudo systemctl start hwp-web (planned)
sudo systemctl status hwp-web (planned)
```
...
| `PORT` | No | `3000` | 서버 포트 |
| `GOOGLE_CLIENT_ID (planned)` | Yes* | - | Google OAuth Client ID |
...
[google]
client_id = "${GOOGLE_CLIENT_ID (planned)}"
...
```bash
# Docker Compose
docker compose up -d --scale hwp-web (planned)=3

# Kubernetes
kubectl scale deployment hwp-web (planned) --replicas=5 -n hwp-bridge

# Cloud Run (자동)
gcloud run services update hwp-web (planned) --max-instances=20
```
...
upstream hwp_backend {
    server hwp-web (planned)-1:3000;
    server hwp-web (planned)-2:3000;
    server hwp-web (planned)-3:3000;
    keepalive 32;
}
```
...
```bash
# 롤백
kubectl rollout undo deployment/hwp-web (planned) -n hwp-bridge

# 특정 리비전으로 롤백
kubectl rollout undo deployment/hwp-web (planned) --to-revision=2 -n hwp-bridge

# 롤백 히스토리
kubectl rollout history deployment/hwp-web (planned) -n hwp-bridge
```
...
```bash
# 이전 리비전으로 트래픽 이동
gcloud run services update-traffic hwp-web (planned) \
  --to-revisions=hwp-web (planned)-00002=100
```
...
```bash
# Docker 로그
docker logs -f hwp-web (planned)

# Kubernetes 로그
kubectl logs -f deployment/hwp-web (planned) -n hwp-bridge

# 컨테이너 진입
docker exec -it hwp-web (planned) /bin/sh
