...
│                                                             │
│  ┌─────────────┐    ┌─────────────┐    ┌─────────────┐     │
│  │   hwp-web (planned)   │───▶│ Prometheus  │───▶│   Grafana   │     │
...
scrape_configs:
  - job_name: 'hwp-web (planned)'
    static_configs:
      - targets: ['hwp-web (planned):3000']
...
```logql
# 에러 로그
{service="hwp-web (planned)"} |= "ERROR"

# 특정 요청 추적
{service="hwp-web (planned)"} |~ "request_id.*abc123"

# 느린 요청 (>1s)
{service="hwp-web (planned)"} | json | parse_time_ms > 1000

# 암호화 문서 업로드
{service="hwp-web (planned)"} |= "Encrypted"

# 최근 1시간 에러 수
count_over_time({service="hwp-web (planned)"} |= "ERROR" [1h])
```
