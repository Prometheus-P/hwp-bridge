...
# 로그 확인
docker logs -f hwp-web (planned) --tail 100
kubectl logs -f deployment/hwp-web (planned) -n hwp-bridge

# 서비스 재시작
docker compose restart hwp-web (planned)
kubectl rollout restart deployment/hwp-web (planned) -n hwp-bridge
...
# 2. 컨테이너 로그 확인
docker logs hwp-web (planned) --tail 50
# 또는
kubectl logs deployment/hwp-web (planned) -n hwp-bridge --tail 50

# 3. 리소스 사용량 확인
docker stats hwp-web (planned)
# 또는
...
```bash
# OOM 확인
docker inspect hwp-web (planned) | jq '.[0].State'
kubectl describe pod -l app=hwp-web (planned) -n hwp-bridge | grep -A5 "Last State"
...
```bash
# 1. 에러 로그 확인
docker logs hwp-web (planned) 2>&1 | grep -i error | tail -20
...
```bash
# 특정 파일 패턴 확인
docker logs hwp-web (planned) 2>&1 | grep ParseError
...
docker compose down
# 환경변수에서 GOOGLE_CLIENT_ID (planned) 제거
docker compose up -d
```
...
# 3. 리소스 사용량
docker stats hwp-web (planned)
top -p $(pgrep hwp-web (planned))

# 4. 대용량 파일 처리 확인
docker logs hwp-web (planned) 2>&1 | grep -E "file_size.*[0-9]{7,}"
```
...
# process_resident_memory_bytes{job="hwp-web (planned)"}

# 2. 힙 프로파일 (개발 환경)
MALLOC_CONF="prof:true" ./hwp-web (planned)
jeprof ./hwp-web (planned) ./heap.prof
